...
- When a finalizer must publish an event so the TUI can drop its local copy of in-memory state, pin the `Location.Info` from the init-time `ctx: InstanceContext` and pass it as `{ location }` to `events.publish`. By the time the finalizer runs, `InstanceRef` is already gone, so the default `EventV2Bridge.publish` falls back to location-less emission and the SSE handler (`server/routes/instance/httpapi/handlers/event.ts`) filters it out. See `permission/index.ts` and `question/index.ts` for the pattern.

...
import { formatDurationMs, toolTiming } from "@opencode-ai/core/util/duration"
...
export function useToolDuration(part: Accessor<ToolPart | undefined>, active: Accessor<boolean>) {
  const timing = createMemo(() => {
    const value = part()
    if (!value) return
    return toolTiming(value.state)
  })
  return useLiveDuration(
    () => timing()?.start,
    () => timing()?.end,
    active,
  )
